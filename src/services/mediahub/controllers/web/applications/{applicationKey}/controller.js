/*!
* /web/applications/{applicationKey}/controller.js
*
...
;!function(undefined) {
	...
	ctrl.get = function(req, res){
		...
		ApplicationModel.find({ "where": { key: req.params.applicationKey }}, function(err, applications){
			...
			if(context.application.primaryCategoryKey)
				context.backButtonLink += "?category=" + context.application.primaryCategoryKey;
			...
			leftnav(isnode, context, function(err, cxt){
				SettingModel.find({where: {setting: "hideRevertToFile"}}, function(err2, settings){
					...
					res.render("applications/application-details.mustache", cxt);
				});
			});
		});
		return;
	...
	ctrl.post = function(req, res){
		var context = {};
		ApplicationModel.find({ "where": { key: req.params.applicationKey }}, function(err1, applications){
			if(!applications){
				res.redirect("/web/applications");
				return;
			}
			FileModel.find({ "where": { key: applications[0].fileKey }}, function(err2, files){
				if(!files){
					res.redirect("/web/applications");
					return;
				}
				if(req.body.revert == "true"){
					var completed = 0;
					applications[0].destroy(function(err3, deletedApplication){
						completed ++;
					});
					FileModel.update({ 
						where: { key: applications[0].fileKey } 
					}, {
						objectType: null,
						objectKey: null
