/*!
* /web/television/{episodeKey}/edit/controller.js
*
...
;!function(undefined) {
	...
	ctrl.get = function(req, res){
		var context = {};
		context.backButtonLink = "/web/television/" + req.params.episodeKey;
		EpisodeModel.find({ "where": { key: req.params.episodeKey }}, function(err, episodes){
			...
			if (context.episode.primaryCategoryKey) {
				CategoryModel.find({ where: { key: context.episode.primaryCategoryKey }}, function(err2, categories) {
	...
	ctrl.post = function(req, res){
		...
		EpisodeModel.update({
			key: req.params.episodeKey
		}, {
			title: req.body.title,
			shortDesc: req.body.shortDesc,
			longDesc: req.body.longDesc,
			primaryCategoryKey: primaryCategoryKey
		}, function(err, updatedEpisode) {
