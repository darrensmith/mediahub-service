...
;!function(undefined) {
	...
	ctrl.get = function(req, res){
		...
		if(!parentCategoryKey) {
			...
			context.categoryLink = "/web/television?category=";
			context.breadcrumbs += " > Television";
			responseCount ++;
			responseCount ++;
		} else {
			CategoryModel.find({ "where": { key: parentCategoryKey, status: "active" }}, function(err,categoriesReturned){
				if(!categoriesReturned || !categoriesReturned[0] || !categoriesReturned[0].parentCategoryKey) {
					...
					context.categoryLink = "/web/television?category=";
					context.breadcrumbs += " > <a href=\"/web/television\" style=\"color:white;\">Television</a> > " + categoriesReturned[0].title;
					responseCount ++;
				} else {
					context.backButtonLink = "/web/television?category=" + categoriesReturned[0].parentCategoryKey;
					CategoryModel.find({ "where": { key: categoriesReturned[0].parentCategoryKey, status: "active" }}, function(err2,categoriesReturned2){
						context.breadcrumbs += " > <a href=\"/web/television\" style=\"color:white;\">Television</a> > <a href=\"/web/television?category=" + categoriesReturned2[0].key + "\" style=\"color:white;\">" + categoriesReturned2[0].title + "</a> > " + categoriesReturned[0].title;
						responseCount ++;
					});
				}
				responseCount ++;
		...
		EpisodeModel.find({ where: { status: "active", primaryCategoryKey: parentCategoryKey }}, function(err, episodes){
		...
		var interval = setInterval(function(){
			if(responseCount >= 4){
				...
				leftnav(isnode, context, function(err, cxt){
					res.render("television/television.mustache", cxt);
				});
			}
		}, 10);
		return;
