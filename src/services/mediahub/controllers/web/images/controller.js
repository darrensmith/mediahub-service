...
;!function(undefined) {
	...
	ctrl.get = function(req, res){
		...
		if(!parentCategoryKey) {
			...
			context.categoryLink = "/web/images?category=";
			...
			CategoryModel.find({ "where": { key: parentCategoryKey, status: "active" }}, function(err,categoriesReturned){
				if(!categoriesReturned || !categoriesReturned[0] || !categoriesReturned[0].parentCategoryKey) {
					...
					context.categoryLink = "/web/images?category=";
					context.breadcrumbs += " > <a href=\"/web/images\" style=\"color:white;\">Images</a> > " + categoriesReturned[0].title;
					responseCount ++;
				} else {
					context.backButtonLink = "/web/images?category=" + categoriesReturned[0].parentCategoryKey;
					CategoryModel.find({ "where": { key: categoriesReturned[0].parentCategoryKey, status: "active" }}, function(err2,categoriesReturned2){
						context.breadcrumbs += " > <a href=\"/web/images\" style=\"color:white;\">Images</a> > <a href=\"/web/images?category=" + categoriesReturned2[0].key + "\" style=\"color:white;\">" + categoriesReturned2[0].title + "</a> > " + categoriesReturned[0].title;
						responseCount ++;
					});
				}
				responseCount ++;
		...
		ImageModel.find({ where: { status: "active", primaryCategoryKey: parentCategoryKey }}, function(err, images){
		...
		var interval = setInterval(function(){
			if(responseCount >= 4){
				...
				leftnav(isnode, context, function(err, cxt){
					res.render("images/images.mustache", cxt);
				});	
			}
		}, 10);
	}
