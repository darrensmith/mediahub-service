...
;!function(undefined) {
	...
	ctrl.get = function(req, res){
		...
		if(!parentCategoryKey) {
			...
			context.categoryLink = "/web/documents?category=";
			context.breadcrumbs += " > Documents";
			responseCount ++;
			responseCount ++;
		} else {
			CategoryModel.find({ "where": { key: parentCategoryKey, status: "active" }}, function(err,categoriesReturned){
				if(!categoriesReturned || !categoriesReturned[0] || !categoriesReturned[0].parentCategoryKey) {
					...
					context.categoryLink = "/web/documents?category=";
					context.breadcrumbs += " > <a href=\"/web/documents\" style=\"color:white;\">Documents</a> > " + categoriesReturned[0].title;
					responseCount ++;
				} else {
					context.backButtonLink = "/web/documents?category=" + categoriesReturned[0].parentCategoryKey;
					CategoryModel.find({ "where": { key: categoriesReturned[0].parentCategoryKey, status: "active" }}, function(err2,categoriesReturned2){
						context.breadcrumbs += " > <a href=\"/web/documents\" style=\"color:white;\">Documents</a> > <a href=\"/web/documents?category=" + categoriesReturned2[0].key + "\" style=\"color:white;\">" + categoriesReturned2[0].title + "</a> > " + categoriesReturned[0].title;
						responseCount ++;
					});
				}
				responseCount ++;
		...
		DocumentModel.find({ where: { status: "active", primaryCategoryKey: parentCategoryKey }}, function(err,documents){
		...
		var interval = setInterval(function(){
			if(responseCount >= 4){
				...
				leftnav(isnode, context, function(err, cxt){
					res.render("documents/documents.mustache", cxt);
				});	
			}
		}, 10);
		return;
