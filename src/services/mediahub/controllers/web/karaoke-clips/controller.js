...
;!function(undefined) {
	...
	ctrl.get = function(req, res){
		...
		if(!parentCategoryKey) {
			...
			context.breadcrumbs += " > Karaoke Clips";
			responseCount ++;
			responseCount ++;
		} else {
			CategoryModel.find({ "where": { key: parentCategoryKey, status: "active" }}, function(err,categoriesReturned){
				if(!categoriesReturned || !categoriesReturned[0] || !categoriesReturned[0].parentCategoryKey) {
					...
					context.categoryLink = "/web/karaoke-clips?category=";
					context.breadcrumbs += " > <a href=\"/web/karaoke-clips\" style=\"color:white;\">Karaoke Clips</a> > " + categoriesReturned[0].title;
					responseCount ++;
				} else {
					...
					CategoryModel.find({ "where": { key: categoriesReturned[0].parentCategoryKey, status: "active" }}, function(err2,categoriesReturned2){
						context.breadcrumbs += " > <a href=\"/web/karaoke-clips\" style=\"color:white;\">Karaoke Clips</a> > <a href=\"/web/karaoke-clips?category=" + categoriesReturned2[0].key + "\" style=\"color:white;\">" + categoriesReturned2[0].title + "</a> > " + categoriesReturned[0].title;
						responseCount ++;
					});
				}
				responseCount ++;
